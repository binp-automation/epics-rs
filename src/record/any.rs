...
use super::{
    RecordType, Record, ReadRecord, WriteRecord,
...
pub enum AnyHandlerBox {
    ...
    // Bi(Box<dyn BiRecord + Send>),
    // Bo(Box<dyn BoRecord + Send>),
    // Longin(Box<dyn LonginRecord + Send>),
    // Longout(Box<dyn LongoutRecord + Send>),
    // Stringin(Box<dyn StringinRecord + Send>),
    // Stringout(Box<dyn StringoutRecord + Send>),
}
impl AnyHandlerBox {
    pub fn rtype(&self) -> RecordType {
        match self {
